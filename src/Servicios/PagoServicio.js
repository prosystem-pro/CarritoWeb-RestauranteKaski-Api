...
const Listado = async (Anio) => {
  ...
  const Resultado = Registros.map(r => {
    const Dato = r.toJSON();
    const fechaLuxon = DateTime.fromJSDate(new Date(Dato.FechaVencimientoPago)).setLocale('es-GT');
    Dato.DiaVencimiento = fechaLuxon.day;
    Dato.MesVencimiento = fechaLuxon.monthLong; 
    Dato.AnioVencimiento = fechaLuxon.year;
